...
def main():
    ...
    val_dataset = _process_caption_data(caption_file='data/annotations/captions_val2014.json',
                                        ...
                                        max_length=max_length)
    #train_cutoff = int(0.7*len(train_dataset))
    ...
    with tf.Session() as sess:
        ...
        for split in [ 'train','val', 'test']:
            ...
            save_path = h5py.File('./data/%s/%s.h5' %(split, split),'w')
...
if __name__ == "__main__":
    #main()
    annotations = load_pickle('./data/coco_data/train/train.annotations.pkl' )
    
    #annotations = load_pickle('./data/coco_data/test/test.annotations.pkl' )
    
    #word_to_idx = load_pickle('./data/train/word_to_idx.pkl' )
#    word_to_idx = _build_vocab(annotations=annotations, threshold=1)
...
#    print len(annotations)
    ...
    with tf.Session() as sess:
        ...
        #save_path = './data/%s/%s.features.hkl' % (split, split)
        save_path = h5py.File('./data/my_pic.h5','w')
        #save_path1 = './data/%s/%s.features1.hkl' % (split, split)#
        
        image_path = list('image/train2014_resized/COCO_train2014_000000013140.jpg')
        image_path = list(annotations['file_name'][:3].unique())
